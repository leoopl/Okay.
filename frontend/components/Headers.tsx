...
export default function Header() {
	...
	return (
		<div className="container mx-auto flex items-center border-b-2 px-6 py-2 h-24">
			...
			<Link href={"/"} className="font-bold text-selectiveyellow">
				Okay!
			</Link>
			<div className="grow">
				<div className="flex items-center justify-center gap-2 md:gap-8">
					{pages.map((page) => (
						<Link href={`/${page.link}`} key={page.link} className="text-selectiveyellow">
							{page.name}
						</Link>
					))}
				</div>
			...
			<div>
				<Link href="/singup" className="mr-2 font-bold text-selectiveyellow">
					Sing Up
				</Link>
				<Link href="/signin" className="font-bold text-selectiveyellow">
					Login
				</Link>
			</div>
		</div>
